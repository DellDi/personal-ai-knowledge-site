...
- 新增 `infra/docker-compose.yml`：postgres + minio + minio-init + cms
...
- [x] `infra/docker-compose.yml` 定义 postgres + minio + cms 服务
- [x] `pnpm --filter @personal-ai-knowledge-site/cms typecheck` 通过
- [ ] `docker compose -f infra/docker-compose.yml up --build` 起 PG + MinIO + CMS（需本地 Docker）
